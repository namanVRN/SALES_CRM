...
function Layout({ children, breadcrumbs }) {
  ...
  // ✅ NEW: Dynamic Step Badge Styling (Handles Red for Not Interested)
  const getStepBadgeStyle = (step, status) => {
    const statusLower = (status || "").toLowerCase();
    
    // Check if Not Interested
    if (statusLower === "not interested") {
      return { bg: "#fee2e2", color: "#dc2626", border: "#fca5a5" }; // Red Badge
    }
    
    // Check if Booked
    if (statusLower === "booked") {
      return { bg: "#d1fae5", color: "#047857", border: "#6ee7b7" }; // Green Badge
    }
  ...
  const handleResultClick = (result) => {
    ...
    if (result.sheetType === "Direct") {
      ...
      const canContact = result.canContact !== "No"; 
              ...
              <div className="search-results-dropdown">
                {isSearching ? (
                  ...
                  <>
                    ...
                    <div className="search-results-list">
                      {searchResults.map((result, index) => {
                        // ✅ Pass result.stepStatus here
                        const stepStyle = getStepBadgeStyle(result.currentStep, result.stepStatus);
